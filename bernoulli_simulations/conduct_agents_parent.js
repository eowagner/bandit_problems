...
function launch_next_child() {
	...
	var parameters = {
		...
		randomize: randomize,
		num_restricted: num_restricted
	};

	proc_index++;

	var child = child_process.fork('./bernoulli_simulations/conduct_agents_child.js');
...
function convert_results_to_string(res) {
	var s = res.parameters.graph.length + "," + res.parameters.p[0] + "," + res.parameters.p[1] + "," + res.success_count;
